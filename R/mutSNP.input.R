mutSNP.input <- function(mut.data, chr = "chr", pos = "pos", ref = "ref", 
    ...
    if (class(mut.data) != "data.frame") {
      stop("mut.data needs to be a dataframe")
    }
    ...
    if (build == "hg19") {
        require_namespace("BSgenome.Hsapiens.UCSC.hg19")
        ...
        require_namespace("BSgenome.Hsapiens.UCSC.hg18")
        ...
        bsg = BSgenome.Hsapiens.UCSC.hg18::BSgenome.Hsapiens.UCSC.hg18
    } else if (build == "hg38") {
        require_namespace("BSgenome.Hsapiens.UCSC.hg38")
